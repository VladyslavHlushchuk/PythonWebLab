...
class AddUpdatePostForm(FlaskForm):
    title = StringField("Заголовок", validators=[DataRequired("Це поле обов'язкове")])
    text = TextAreaField("Зміст посту")
    image = FileField('Додати зображення до посту', validators=[FileAllowed(['jpg', 'png'])])
